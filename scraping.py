...
def scrape_all():
    ...
    data = {
        ...
        "hemispheres": mars_hemisphere_url_title
...
def mars_hemisphere_url_title(browser):
    ...
    # Write code to retrieve the image urls and titles for each hemisphere.
        for i in range(0,4):
            ...
            full_image = jpg_soup.find_all('div',"description")[i]
            find_url = full_image.find('a').get('href')
            find_title = full_image.find('h3').text
            final_url = f'{url}{find_url}'
            browser.visit(final_url)
            html = browser.html
            image_soup = soup(html, 'html.parser')
            title_elem = image_soup.find('div',"downloads")
            full_image_url  = title_elem.find('a').get('href')
            final_url = f'{url}{full_image_url}'
            hemisphere_image_urls.append({'img_url':final_url,'title':find_title})
